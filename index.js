...
function printUppercase(array) {
    for (let m = 0; i < array.length; m++) {
        console.log(array(m).toUpperCase());
    }
...
function concatenateStrings(array, subString) {
    const concatenateStrings = array.toString();
    const containsSubstring = concatenateStrings.includes(subString);
...
function getEvenNumbers(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i] % 2 === 0) {
            result = [...result, arr[i]]; // or use result.push(arr[i]);
        }
    }
...
function multiplyByIndex(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        result = [...result, arr[i] * i]; // ir use result.push(arr[i] * i);
    }
